...
class MinidumpAllocator {
 ...
 public:
  explicit MinidumpAllocator(uint32_t size) :
    ...
    // ARC MOD BEGIN
    // Use calloc() since PluginInstance may not be ready to call mmap() here.
    void *mapping = calloc(1, size);
    // ARC MOD END
  ...
  ~MinidumpAllocator() {
    if (buf_ != NULL) {
      // ARC MOD BEGIN
      // Call free() instead of munmap() because we allocates it with calloc()
      // instead of mmap().
      free(buf_);
      // ARC MOD END
      // ARC MOD BEGIN UPSTREAM chromium-fix-unused-var
      // Benign reference for the opt case when the assert compiles away.
      // (void) result;
      // ARC MOD END UPSTREAM
    }
  }
...
static void WriteSystemInfo(MinidumpAllocator *minidump_writer,
  ...
  // ARC MOD BEGIN
  // Add information about whether this dump comes from NaCl or BareMetal.
#if defined(__native_client__)
  sysinfo.get()->csd_version_rva = WriteString(minidump_writer, "nacl");
#else
  sysinfo.get()->csd_version_rva = WriteString(minidump_writer, "nonsfi-nacl");
#endif
  // ARC MOD END
...
// ARC MOD BEGIN UPSTREAM chromium-minidump-null-dlpiname
  if (info->dlpi_name == NULL ||
      strlen(info->dlpi_name) == 0) {
    module->name = modules_arena->StrDup("unknown");
  } else if (strcmp(info->dlpi_name, "/lib/main.nexe") == 0) {
...
static void WriteMemoryInfoList(MinidumpAllocator *minidump_writer,
  ...
  /* ARC MOD BEGIN UPSTREAM chromium-minidump-on-arm */
  // Use memset as number_of_entries may not be aligned on ARM.
  memset(&(memory_info_list.get()->number_of_entries), 0, sizeof(uint64_t));
  /* ARC MOD END UPSTREAM */
...
void nacl_minidump_register_crash_handler(void) {
  ...
  if (errno != 0) {
    // ARC MOD BEGIN
    // TODO(crbug.com/363501): perror seems to crash at this point.
    arc::WriteLog("minidump: Failed to register an exception handler\n");
    // ARC MOD END
    return;
  }
