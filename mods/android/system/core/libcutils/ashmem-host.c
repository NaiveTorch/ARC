...
/* ARC MOD BEGIN */
// TODO(crbug.com/379838): Remove once the bug is fixed.
...
retry:
	...
	/* ARC MOD BEGIN */
	// Append the incoming debug name in |ignored| if it is provided.
	// The name will be shown up when ARC strace is enabled.
	char* out = name + strlen(name);
	if (ignored != NULL) {
		const char* extra = ignored;
		while (*extra && out - name < sizeof(name) - 1) {
			*out++ = isprint(*extra) && *extra != '/' ?
			  *extra : '_';
			++extra;
		}
		*out = '\0';
	} else {
		// It is possible to have NULL names for ashmem regions. Just
		// indicate that.
		const char kNULL[] = "NULL";
		if (out + sizeof(kNULL) < name + sizeof(name))
			strcpy(out, kNULL);
	}
...
{
        /* ARC MOD BEGIN */
        // TODO(crbug.com/379838): Implement this.
        ALOGW("ashmem_set_prot_region: not implemented: fd=%d, prot=%d",
              fd, prot);
        /* ARC MOD END */
	return 0;
}
...
int ashmem_pin_region(int fd, size_t offset, size_t len)
{
        /* ARC MOD BEGIN */
        // TODO(crbug.com/379838): Implement this.
        ALOGW("ashmem_pin_region: not implemented: fd=%d, offset=%zu, len=%zu",
              fd, offset, len);
        /* ARC MOD END */
...
int ashmem_unpin_region(int fd, size_t offset, size_t len)
{
        /* ARC MOD BEGIN */
        // TODO(crbug.com/379838): Implement this.
        ALOGW("ashmem_unpin_region: not implemented: fd=%d, offset=%zu, len=%zu",
              fd, offset, len);
        /* ARC MOD END */
