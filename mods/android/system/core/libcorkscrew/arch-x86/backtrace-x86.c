...
/* ARC MOD BEGIN */
/* ucontext_t is defined in glibc in sys/ucontext.h,
 * which is included from various standard headers.
 */
...
ssize_t unwind_backtrace_signal_arch(siginfo_t* siginfo __attribute__((unused)), void* sigcontext,
  ...
  /* This code path is disabled for x86_64 Native Client.
   *
   * Native Client achieves improved source-code portability by
   * using 32-bit int, long and pointer types for all platforms,
   * including x86_64. Source code portability breaks down for code
   * written in assembler, code that is aware of the ABI, and code
   * that manipulates register state. For this reason the unwind
   * paths in this file are disabled. At some point in the future
   * these paths will need to be reimplemented for the x86_64 ABI.
   *
   * Also note that unwind_backtrace_signal_arch is likely not
   * useful until Native Client adds support for signals.
   */
  return 0;
#else
  /* ARC MOD END */
...
#if defined(__native_client__) && defined(__x86_64__)
  /* This code path is disabled for x86_64 Native Client.
   * Please see the comment above for unwind_backtrace_signal_arch().
   */
  return 0;
#else
  /* ARC MOD END */
