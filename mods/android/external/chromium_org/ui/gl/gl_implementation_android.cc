...
bool InitializeGLBindings(GLImplementation implementation) {
  ...
  switch (implementation) {
    case kGLImplementationEGLGLES2: {
      /* ARC MOD BEGIN */
      // The names of the .so in our implementation of GL are different from
      // Android's.
      base::NativeLibrary gles_library = LoadLibrary("libGLESv2_emulation.so");
      /* ARC MOD END */
      if (!gles_library)
        return false;
      /* ARC MOD BEGIN */
      base::NativeLibrary egl_library = LoadLibrary("libEGL_emulation.so");
      /* ARC MOD END */
      if (!egl_library) {
