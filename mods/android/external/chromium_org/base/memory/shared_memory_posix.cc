...
bool SharedMemory::MapAt(off_t offset, size_t bytes) {
  ...
  /* ARC MOD BEGIN */
  // If we are mapping the file from the beginning, map the entire file
  // everytime.  This keeps a consistent mapping for the entire file and avoids
  // overlapping mmap regions, which posix_translation does not handle.
  // TODO(crbug.com/344086): Remove this if/when we support overlapping mmaps.
  if (offset == 0) {
    int ashmem_bytes = ashmem_get_size_region(mapped_file_);
    if (ashmem_bytes >= 0)
      bytes = ashmem_bytes;
