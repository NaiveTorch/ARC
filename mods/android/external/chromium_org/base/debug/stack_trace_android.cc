...
// ARC MOD BEGIN UPSTREAM chromium-add-missing-include
#include <signal.h>
...
void StackTrace::OutputToStream(std::ostream* os) const {
  ...
  // UI thread.
  // ARC MOD BEGIN
  // We do not use Chromium's backtracer.
#if defined(OS_NACL)
  *os << "stack_trace_android.cc is not supported. "
      << "Use src/common/backtrace.h instead.\n";
#else
  // ARC MOD END
