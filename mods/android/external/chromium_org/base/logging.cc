...
#endif
    // ARC MOD BEGIN
    // Remove the redundant fprintf. Since we always use OS_ANDROID, and on
    // ARC, the __android_log_write call above writes the log to stderr, the
    // fprintf/fflush calls below are completely unnecessary. Writing logs
    // without using __android_log_write may confuse our function wrapping
    // system. For example, calling this function (i.e. LOG(ERROR) << "..." or
    // CHECK(...)) inside posix_translation/ may cause dead lock if we keep the
    // fprintf.
