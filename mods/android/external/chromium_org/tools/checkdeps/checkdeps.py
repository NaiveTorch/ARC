...
# ARC MOD BEGIN
# Add a search path. Otherwise Python will only check
# mods/android/external/chromium_org/tools/checkdeps/.
sys.path.insert(0, 'out/staging/android/external/chromium_org/tools/checkdeps/')
# ARC MOD END
import cpp_checker
import java_checker
import results
...
class DepsChecker(object):
  ...
  def _ApplyDirectoryRules(self, existing_rules, dir_name):
    ...
    if not (os.path.exists(os.path.join(dir_name, ".svn")) or
      ...
      # ARC MOD BEGIN
      # The 'norm_dir_name in ..' check only works when the root directory has
      # .git, but in our case, the directory does not have it.
      pass  # check all sub directories which is safe in our case.
      # return (None, [])
      # ARC MOD END
