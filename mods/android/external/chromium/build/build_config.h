...
/* ARC MOD BEGIN */
// Add defined(ANDROID) so that the subsequent macros are defined even when we
// are building code with NaCl toolchains, where OS_LINUX is not defined.
#if defined(OS_LINUX) || defined(OS_FREEBSD) || defined(OS_OPENBSD) || \
    defined(OS_SOLARIS) || defined(ANDROID)
/* ARC MOD END */
...
// Use tcmalloc
/* ARC MOD BEGIN */
// Add defined(OS_ANDROID) so that the subsequent macro is defined even when we
// are building code with NaCl toolchains, where OS_LINUX is not defined.
...
// Add defined(OS_ANDROID) so that the subsequent macro is defined even when we
// are building code with NaCl toolchains, where OS_LINUX is not defined.
#if (defined(OS_LINUX) || defined(OS_ANDROID)) && !defined(NO_HEAPCHECKER)
/* ARC MOD END */
