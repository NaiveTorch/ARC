...
int GetProcessCPU(pid_t pid) {
  /* ARC MOD BEGIN */
  // /proc does not currently work on NaCl, returning failure.
  // Gating out the entire function as it references base::ParseProcStatCPU,
  // which doesn't seem to validate.
#if defined(OS_NACL)
  return -1;
#else
  /* ARC MOD END */
