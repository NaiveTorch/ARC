...
namespace CPPUNIT_NS
{
  ...
  public:
    ...
    /* ARC MOD BEGIN */
    // Following modification is for supporting multiple exclude/include rules.
    // Arguments:
    // - in_desiredTests: a comma separated list of exclude rules or include
    //                    rules.
    //                    (e.g. "HogeTest" or "HogeTest,FugaTest::testcase)
    // - is_exclude_rule: True if the |in_desiredTest| is exclude rule,
    //                    otherwise |in_desiredTest| is include rule.
    // - explicit_test: The "explicit test" doesn't run by default, only runs
    //                  with specifying with include rule.
    // For example, to exclude "HogeTest" and "FugaTest::testcase1":
    // $ ./stlport_unittests -x=HogeTest,FugaTest::testcase1
    bool shouldRunThis(const char *in_desiredTests, const char *in_className, const char *in_functionName,
                       bool is_exclude_rule, bool explicit_test, bool &do_progress) {
      char* buffer = strdup(in_desiredTests);
      static const char delimiter[] = ",";
      for (char* token = strtok(buffer, delimiter); token != NULL; token = strtok(NULL, delimiter)) {
        const bool should_run = shouldRunThisInternal(token, in_className, in_functionName, is_exclude_rule,
                                                      explicit_test, do_progress);
        if (should_run && !is_exclude_rule) {
          free(buffer);
          return true;
        } else if (!should_run && is_exclude_rule) {
          free(buffer);
          return false;
        }
      }
      ...
      free(buffer);
      // If no rule is matched, run this test if passed rule is exclude rule.
      return is_exclude_rule;
    }

    bool shouldRunThisInternal(const char *in_desiredTest, const char *in_className, const char *in_functionName,
