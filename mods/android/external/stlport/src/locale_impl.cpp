...
// We build STLPort as a static library for libposix_translation.so. Do not
// initialize ios (cin, cout, and cerr) here when it is build as a static
// library. Here is why:
// * _IosInit's constructor calls fstats. In posix translation, fstats (and
//   other calls) do not directly call into IRT, but calls into ARC version
//   of the IRT in wrap/. However, when the static initializer is called,
//   ARC version of the IRT has not been set up yet (i.e. DO_WRAP has not
//   been called).
// * _IosInit's destructor calls fflush. This is not allowed in posix
//   translation and aborts in libc_dispatch_layer.cc.
