// ARC MOD TRACK "third_party/nacl-glibc/sysdeps/nacl/irt_dev.h"
/*
...
struct nacl_irt_dev_fdio {
  ...
  // ARC MOD BEGIN UPSTREAM nacl-use-abi
  int (*ftruncate)(int fd, nacl_abi_off_t length);
  // ARC MOD END UPSTREAM
};
...
struct nacl_irt_dev_filename {
  ...
  int (*unlink)(const char *pathname);
  // ARC MOD BEGIN UPSTREAM nacl-use-abi
  int (*truncate)(const char *pathname, nacl_abi_off_t length);
  int (*lstat)(const char *pathname, struct nacl_abi_stat *);
  // ARC MOD END UPSTREAM
