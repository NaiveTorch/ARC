...
static int __bionic_open_tzdata(const char* olson_id, int* data_size) {
  ...
  // ARC MOD BEGIN
  // Do not access /data/misc/zoneinfo/tzdata for better performance.
  // On ARC, the file does not exist.
  int fd = -1;  // __bionic_open_tzdata_path("/data/misc/zoneinfo/tzdata", olson_id, data_size);
  // ARC MOD END
  if (fd < 0) {
