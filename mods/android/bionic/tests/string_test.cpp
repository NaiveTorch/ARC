...
// ARC MOD BEGIN UPSTREAM bionic-add-strcpy-test

// Adds some more tests.
// TODO(yusukes): The upstream KitKat has added lots of tests to this file. Consider removing
// some of ours.
TEST(string, strcpy_) {
  char buf[3];
  strcpy(buf, "");
  ...
  // The strcpy implementation tries to align the source pointer to an 8 byte
  // boundary before entering a loop. Confirm that the function does not crash
  // regardless of whether the initial address of the source string.
  static const char* teststr = "012345678";
  char buf2[10];
  for (size_t i = 0; i < 9; ++i) {
    strcpy(buf2, teststr + i);
    EXPECT_EQ(static_cast<char>('0' + i), buf2[0]);
  }
}
...
TEST(string, strlen_) {
  EXPECT_EQ(0U, strlen(""));
  EXPECT_EQ(1U, strlen("1"));
  EXPECT_EQ(2U, strlen("12"));
  EXPECT_EQ(3U, strlen("123"));
  // The strlen_ implementation processes 32 bytes per loop. Tests with a
  // string with >32 chars just in case.
  EXPECT_EQ(90U, strlen("012345678901234567890123456789"
                        "012345678901234567890123456789"
                        "012345678901234567890123456789"));
}

TEST(string, strcmp_) {
  static const char teststr1[] = "012345678901234567890123456789012345678901"
    "012345678901234567890123456789012345678901"
    "012345678901234567890123456789012345678901"
    "012345678901234567890123456789012345678901";
  static const char teststr2[] = "_012345678901234567890123456789012345678901"
    "012345678901234567890123456789012345678901"
    "012345678901234567890123456789012345678901"
    "012345678901234567890123456789012345678901";
  // Tests strcmp with various lengths and pointer alignments.
  for (size_t i = 0; i < sizeof(teststr1); ++i) {
    ASSERT_NE(0, strcmp(&teststr1[i], &teststr2[i])) << i;
    ASSERT_EQ(0, strcmp(&teststr1[i], &teststr2[i + 1])) << i;
  }
}
...
TEST(string, memcmp_) {
  ...
  static const char teststr2[] = "_012345678901234567890123456789012345678901"
    ...
    "012345678901234567890123456789012345678901"
    "012345678901234567890123456789012345678901";
  static const size_t teststr1_len = sizeof(teststr1);

  // Tests memcmp with various lengths and pointer alignments.
  for (size_t i = 0; i < teststr1_len; ++i) {
    ASSERT_NE(0, memcmp(&teststr1[i], &teststr2[i], teststr1_len - i)) << i;
    ASSERT_EQ(0, memcmp(&teststr1[i], &teststr2[i + 1], teststr1_len - i)) << i;
  }
}

TEST(string, memset_) {
  char test[128];
  // Tests memset with various lengths and pointer alignments.
  for (size_t i = 0; i < sizeof(test) - 1; ++i) {
    // Reset the array.
    for (size_t j = 0; j < sizeof(test) - 1; ++j) {
      test[j] = 255;
    }
    // Fill test[i] to test[127] with c.
    const char c = '0' + (i % 10);
    void* p = memset(&test[i], c, sizeof(test) - i);
    ASSERT_EQ(p, (void*)&test[i]) << i;
    // Verify test[i] to test[127].
    for (size_t j = i; j < sizeof(test) - 1; ++j) {
      const char expected = (j < i ? 255 : c);
      ASSERT_EQ(expected, test[j]) << i << ',' << j;
    }
  }
}
...
TEST(string, bzero_) {
  ...
  for (size_t i = 0; i < sizeof(test) - 1; ++i) {
    ...
    // Fill test[i] to test[127] with 0.
    bzero(&test[i], sizeof(test) - i);
    // Verify test[i] to test[127].
    for (size_t j = 0; j < sizeof(test) - 1; ++j) {
      const char expected = (j < i ? 1 : 0);
      ASSERT_EQ(expected, test[j]) << i << ',' << j;
    }
  }
}
