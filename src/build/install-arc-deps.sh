#!/bin/bash -e
# ARC MOD TRACK "third_party/chromium-ppapi/build/install-build-deps.sh"
...
if egrep -q '14\.04' /etc/issue; then
  echo "ERROR: install-arc-deps.sh is no longer used on Ubuntu 14.0.4" >&2
  echo "ERROR: Use install-build-deps.sh in the same directory instead" >&2
  exit 1
fi
# ARC MOD END
...
# ARC MOD BEGIN
arc_dev_list="build-essential g++-multilib git lib32bz2-dev lib32z-dev
              libosmesa6 libosmesa6:i386 openjdk-6-jdk pbzip2 pigz python-markdown"
dev_list="${dev_list} ${arc_dev_list}"
# ARC MOD END
...
# ARC MOD BEGIN
arc_lib_list="libcomerr2 libdbus-1-3 libdbusmenu-glib4 libdbusmenu-gtk4
                libgconf-2-4 libgcrypt11 libgdk-pixbuf2.0-0 libgl1-mesa-glx
                libgnutls28 libgssapi-krb5-2 libk5crypto3 libkrb5-3 libxss1"
lib_list="${lib_list} ${arc_lib_list}"
# ARC MOD END
...
EOF
  ...
  for i in ${lib_list} ${dbg_list}; do
    orig="$(echo "${tmp}/staging/${i}"_*_i386.deb)"
    compat="$(echo "${orig}" |
    ...
    msg="$(fakeroot -u sh -exc '
      ...
      mkdir -p "'"${tmp}"'/staging"
      cd "'"${tmp}"'/staging"
      apt-get -c="'"${tmp}/apt/apt.conf"'" --yes --force-yes download "'"$i"'"
      ar x *
      mkdir -p dpkg/DEBIAN

      # Though most .deb put data in data.tar.gz, some of them put in
      # data.tar.bz2 or data.tar.xz.  Luckily, modern tar can detect
      # compression format itself, so we do not need to have lots of condition
      # here.
...
# We need libgnutls28:i386 explicitly so it will be installed to the canonical
# location.  Without it, Chrome fails to load without adding
# /usr/lib32/i386-linux-gnu to LD_LIBRARY_PATH.
sudo apt-get install libgnutls28:i386
# libgnutls28 depends on these libraries, but somehow repackaging them into
# ia32 packages doesn't work.  This is a workaround to fix build break.
# TODO(victorhsieh): think about a better solution for this.
sudo apt-get install libnettle4:i386 libhogweed2:i386 libgmp10:i386
# For bare_metal_arm target. gdb-multiarch is required for bare_metal_arm to
# support "--gdb=plugin --remote=..."
sudo apt-get install \
  g++-arm-linux-gnueabihf g++-4.6-multilib-arm-linux-gnueabihf \
  qemu qemu-user gdb-multiarch

# We create a symbolic link to mesa's libGL.so. If you run this script
# on goobuntu workstation, this symlink will be overwritten by
# /usr/lib32/nvidia-current/libGL.so in the loop below.
sudo ln -sf /usr/lib32/i386-linux-gnu/mesa/libGL.so.1 /usr/lib32/libGL.so

for i in $(find /lib32/i386-linux-gnu /usr/lib32/i386-linux-gnu \
                /usr/lib32/nvidia-current -maxdepth 1 -name \*.so.\* |
           sed -e 's/[.]so[.][0-9].*/.so/' |
           sort -u); do
  j="$(ls "$i."* | sed -e 's/.*[.]so[.]\([^.]*\)$/\1/;t;d' |\
       sort -n | tail -n 1)"
  k="$(echo $i | sed -e 's#/i386-linux-gnu##' | sed -e 's#/nvidia-current##' )"
  # Add symlinks e.g. from /usr/lib32/libX11.so
  # to /usr/lib32/i386-linux-gnu/libX11.so.6.
  sudo ln -sf "$i.$j" "$k"
done

# We create the symlink to libcrypto, which is referenced from target_arch=ia32
# Chrome cross compile.
sudo ln -sf /lib/i386-linux-gnu/libcrypto.so.1.0.0 /lib/i386-linux-gnu/libcrypto.so
