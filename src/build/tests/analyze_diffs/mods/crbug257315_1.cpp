...
void dvmCallJNIMethod(const u4* args, JValue* pResult, const Method* method, Thread* self) {
    ...
    /* ARC MOD BEGIN */
    // TODO(crbug.com/242363): All changes in this file are for hard-fp ARM, but
    // they are quick hacks and not really upstream-able. We should instead
    // modify vm/arch/arm/CallEABI.S.
    ...
    if (!method->noRef)
#else
    // hard-fp ARM. Always go into the while-loop to count the number of
    // elements in modArgs[].
#endif
    {
    /* ARC MOD END */
    ...
    /* ARC MOD BEGIN */
#if defined(__arm__) && defined(__VFP_FP__) && !defined(__SOFTFP__)
    // When --mfloat-abi=hard (or -mhard-float) is set, we should use s0-15 (or
    // its alias: d0-7) VFP registers for passing float and double arguments.
    // For more details, see
    // http://infocenter.arm.com/help/topic/com.arm.doc.ihi0042d/IHI0042D_aapcs.pdf
    // (5.1.2.1, 6.1.2.2, and 6.1.2.3).
    static const size_t kNumVfpRegisters = 16;
    size_t vfp_idx = 0;
    bool vfp_in_use[kNumVfpRegisters] = {};
    u4 vfp_reg_value[kNumVfpRegisters];
    {
        const char* shorty = &method->shorty[1]; /* skip return type */
	const int argc = idx;
	idx = (accessFlags & ACC_STATIC) ? 0 : 1;
        // Move float/double arguments from modArgs[] to vfp_reg_value[].
        // vfp_reg_value[] values are later moved to s0-s15 floating point
        // registers.
        while (*shorty != '\0') {
	  switch (*shorty++) {
	  case 'F':
            // TODO(crbug.com/242363): Handle |vfp_index| overflow correctly.
	    if (vfp_idx < kNumVfpRegisters) {
	      vfp_in_use[vfp_idx] = true;
	      vfp_reg_value[vfp_idx] = modArgs[idx];
	      vfp_idx++;
	      for (int i = idx; i < argc - 1; ++i) {
		modArgs[i] = modArgs[i + 1];
	      }
	    }
	    break;
	  case 'D':
	    if (vfp_idx < kNumVfpRegisters - 1) {
	      if (vfp_idx % 2)
		++vfp_idx;  // align to the next dN register boundary.
	      vfp_in_use[vfp_idx] = vfp_in_use[vfp_idx + 1] = true;
	      vfp_reg_value[vfp_idx] = modArgs[idx];
	      vfp_reg_value[vfp_idx + 1] = modArgs[idx + 1];
	      vfp_idx += 2;
	      for (int i = idx; i < argc - 2; ++i) {
		modArgs[i] = modArgs[i + 2];
	      }
	    }
	    break;
	  default:
...
#if defined(__arm__) && defined(__VFP_FP__) && !defined(__SOFTFP__)
// Copies vfp_reg_value[N] to sN register.
#define COPY_TO_VFP_REG(N) \
    if (vfp_in_use[N]) \
      asm __volatile__("fmsr s" #N ", %0" ::"r"(vfp_reg_value[N]))
...
#if defined(__arm__) && defined(__VFP_FP__) && !defined(__SOFTFP__)
    // When the return type of the JNI method is jdouble or jfloat, it's
    // returned via d0/s0, not via the stack.
    if (method->shorty[0] == 'D') {
      void* addr = &(pResult->d);
      asm __volatile__("vstr d0, [%0]" ::"r"(addr) :"memory");
    } else if (method->shorty[0] == 'F') {
      void* addr = &(pResult->f);
      asm __volatile__("vstr s0, [%0]" ::"r"(addr) :"memory");
    }
#endif
