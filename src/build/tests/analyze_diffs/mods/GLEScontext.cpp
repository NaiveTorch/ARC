...
void GLEScontext::init() {
    ...
    if (!m_initialized) {
        ...
        s_glDispatch.glGenBuffers(4 + s_glSupport.maxVertexAttribs,
                                  m_vertexBuffers);
        m_vertexBufferMap[GL_COLOR_ARRAY] = m_vertexBuffers[0];
        m_vertexBufferMap[GL_NORMAL_ARRAY] = m_vertexBuffers[1];
        m_vertexBufferMap[GL_VERTEX_ARRAY] = m_vertexBuffers[2];
        m_vertexBufferMap[GL_TEXTURE_COORD_ARRAY] = m_vertexBuffers[3];
        for (int i = 0; i < s_glSupport.maxVertexAttribs; ++i) {
          m_vertexBufferMap[i] = m_vertexBuffers[4 + i];
        }
        s_glDispatch.glGenBuffers(1, &m_elementArrayBuffer);
#endif
...
GLEScontext::~GLEScontext() {
    ...
    if (m_initialized) {
        ...
        s_glDispatch.glDeleteBuffers(1, &m_elementArrayBuffer);
...
GLint GLEScontext::getTotalArraySize(
    GLint vertex_size, GLenum type, GLsizei stride, GLsizei count) {
    if (stride == 0) {
      stride = getGLTypeSize(type) * vertex_size;
    }
    return stride * count;
}

void GLEScontext::setupArraysPointers(
    GLESConversionArrays& fArrs, GLint first, GLsizei count, GLenum type,
    const GLvoid* indices, bool direct) {
    int countOfVertices;
    if (direct) {
      countOfVertices = count + first;
    } else {
      // TODO(lpique): This findMaxIndex call can be expensive. Eliminate
      // another existing call being made in GLESContext::convertIndirect()
      // which may also be called after this point from needConvert().
      countOfVertices = findMaxIndex(count,type,indices) + 1;
    }
    setupArraysPointersInternal(fArrs, first, count, type,
                                indices, direct, countOfVertices);
#if USE_GL_BUFFER_OBJECTS
    if (indices) {
        s_glDispatch.glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_elementArrayBuffer);
        s_glDispatch.glBufferData(GL_ELEMENT_ARRAY_BUFFER, getTotalArraySize(1, type, 0, count), indices, GL_DYNAMIC_DRAW);
    } else {
        s_glDispatch.glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
    }
#endif
}

void GLEScontext::setArrayBufferIfNecessary(
    const GLvoid** arr, GLenum arrayType, GLenum dataType, GLint size,
    GLsizei stride, GLint countOfVertices) {
#if USE_GL_BUFFER_OBJECTS
    if (m_vertexBufferMap.find(arrayType) != m_vertexBufferMap.end()) {
        GLsizeiptr total_size = getTotalArraySize(size, dataType, stride, countOfVertices);
        s_glDispatch.glBindBuffer(GL_ARRAY_BUFFER, m_vertexBufferMap[arrayType]);
        s_glDispatch.glBufferData(GL_ARRAY_BUFFER, total_size, *arr, GL_DYNAMIC_DRAW);
        // Override arr to zero because subsequent gl*Pointer calls will be
        // referring to this bound buffer and not the client side array.
        *arr = (GLvoid*)0;
    }
#endif
}
...
void GLEScontext::setViewport(GLint x, GLint y, GLsizei width, GLsizei height) {
    if (width < 0 || height < 0) {
        return;  // Such values will produce a GL error.
    }
    m_viewport[0] = x;
    m_viewport[1] = y;
    m_viewport[2] = width;
...
void GLEScontext::initCapsLocked(const GLubyte * extensionString)
{
    ...
    // core FBO support.
    const char* version = (const char*)s_glDispatch.glGetString(GL_VERSION);
    if (version != NULL && strncmp(version, "OpenGL ES 2.", 12) == 0) {
        // GL ES version 2+ has built in framebuffer support.
...
bool GLEScontext::glGetIntegerv(GLenum pname, GLint *params)
{
    switch(pname)
    {
        ...
        case GL_VIEWPORT:
            if (m_viewport[2] == -1) {
                // Invalid 'width' means we had no calls to glViewport().
                return false;
            }
            memcpy(params, m_viewport, sizeof(m_viewport));
            break;
        /* ARC MOD END UPSTREAM */
        default:
            return false;
