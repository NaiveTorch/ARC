...
class IsLaunchChromeCommandTest(unittest.TestCase):
  def test_xvfb(self):
    ...
    self.assertTrue(build_common.is_launch_chrome_command(args))
    self.assertEqual(['run', 'this-app.apk', '--with-this-option'],
                     build_common.remove_leading_launch_chrome_args(args))

    args = ['xvfb-run', '--auto-servernum', '--server-args',
            '-screen 0 640x480x24',
            '--error-file', '/some/output.log', 'do-something-else']
    self.assertFalse(build_common.is_launch_chrome_command(args))
